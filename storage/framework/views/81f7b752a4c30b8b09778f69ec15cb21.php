...
<div class="page-fade-in">
            ...
            <form method="GET" action="" class="filter-wrapper shadow-lg">
                <div class="input-group">
                    ...
                    <input type="text" name="search" class="form-control" 
                           placeholder="Cari nama peminjam..." value="<?php echo e(request('search')); ?>">
                    ...
                    <select name="status" class="form-select" style="max-width: 180px;">
                        ...
                        <option value="belum_dibayar" <?php echo e(request('status') === 'belum_dibayar' ? 'selected' : ''); ?>>Belum Lunas</option>
                        <option value="sudah_dibayar" <?php echo e(request('status') === 'sudah_dibayar' ? 'selected' : ''); ?>>Lunas</option>
                    </select>
                        ...
                        <tr class="table-row-luxury">
                            <td class="ps-4">
                                <div class="d-flex align-items-center">
                                    <div class="avatar-circle me-3">
                                        <?php echo e(strtoupper(substr($fine->loan->user->name ?? '?', 0, 1))); ?>

                                    </div>
                                    <div>
                                        <div class="fw-bold text-dark"><?php echo e($fine->loan->user->name ?? '-'); ?></div>
                                        <small class="text-muted">ID Pinjam: #<?php echo e($fine->loan_id); ?></small>
                                    </div>
                                </div>
                            </td>

                            <td>
                                <div class="fw-semibold text-dark"><?php echo e($fine->loan->book->title ?? '-'); ?></div>
                                <small class="text-muted"><?php echo e($fine->loan->book->author ?? '-'); ?></small>
                            </td>

                            <td class="text-center">
                                <span class="fw-bold text-dark">
                                    Rp<?php echo e(number_format($fine->amount, 0, ',', '.')); ?>

                                </span>
                            </td>

                            <td class="text-center">
                                <?php if($fine->status === 'sudah_dibayar'): ?>
                            ...
                            <td class="text-end pe-4">
                                <div class="btn-group shadow-sm rounded-3">
                                    <?php if($fine->status === 'belum_dibayar'): ?>
                                        <a href="<?php echo e(route('admin.fines.pay', $fine->id)); ?>"
                                           class="btn btn-success btn-sm px-3 border-0 rounded-start-3" title="Bayar Sekarang">
                                            <i class="bi bi-credit-card"></i>
                                        </a>
                                    <?php endif; ?>
                                    
                                    <a href="<?php echo e(route('admin.fines.edit', $fine->id)); ?>"
                                       class="btn btn-outline-warning btn-sm px-3 border-0 <?php echo e($fine->status === 'sudah_dibayar' ? 'rounded-3' : 'rounded-end-3'); ?>" 
                                       title="Edit Data">
    ...
    <div class="mt-4 d-flex justify-content-center">
        <?php echo e($fines->appends(request()->query())->links('pagination::bootstrap-5')); ?>

    </div>
</div>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\AINANHAMMAL\perpustakaan-api\resources\views/admin/fines/index.blade.php ENDPATH**/ ?>